...
<div class="row" style="margin:0.5rem;">
    <div class="row" style="align-items:center;">
        ...
        {/if}
        <input class="value {number_edit_modes.includes($mode) ? 'editable' : ''}" disabled={!number_edit_modes.includes($mode)} on:change={updateDatabase} bind:value={number} placeholder={placeholder}/>
        {#if incremental && number_edit_modes.includes($mode)}
            <button class="custom-box custom-button custom-tiny-button" on:click={() => {
                number++;
                updateDatabase();
    ...
    <Divider/>
    <input class="custom-title {bold_label ? 'bold' : 'not-bold'} {label_edit_modes.includes($mode) ? 'editable' : ''}" disabled={!label_edit_modes.includes($mode)} on:change={updateDatabase} bind:value={label} placeholder={label_placeholder}/>
</div>
